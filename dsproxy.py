...
async def deepseek_response_proxy(conversation_history, lang="jp", timeout: int = 30, jobinfo=None):
    ...
    async with _semaphore:
        ...
        logger.info(
            f"[{request_id}] Acquired semaphore after {wait_time:.2f}s, processing request..."
        )
        ...
        try:
            ...
            response = await asyncio.wait_for(
                asyncio.to_thread(deepseek_chain.invoke, message_list), timeout=timeout
            )

            total_time = time.time() - start_time
            logger.info(
                f"[{request_id}] Request completed successfully in {total_time:.2f}s"
            )
